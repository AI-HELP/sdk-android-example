...
class OthersFragment : Fragment(), View.OnClickListener {
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.btn_unread_msg -> AIHelpSupport.startUnreadMessageCountPolling { msgCount ->
                Toast.makeText(
                    context,
            ...
            R.id.btn_network_check -> AIHelpSupport.setNetworkCheckHostAddress("aihelp.net") { netLog ->
                Toast.makeText(
                    context,
                    "Check Result: $netLog",
                    Toast.LENGTH_SHORT
